...
window_name = "Google Slides Gesture Control"
...
def send_keystroke(action, *keys):
    try:
        if action == "press":
        ...
        print(f"PyAutoGUI error sending {keys}: {exc}")
...
while True:
    ...
    if detection_result.hand_landmarks:
        ...
        for hand_landmarks in detection_result.hand_landmarks:
            ...
            gesture_details = f"Fingers: {fingers} total={total}"
            ...
            if current_time - last_time > cooldown:
                ...
                if total == 4:
                    send_keystroke("press", "right")
                    gesture = "Next Slide"
                    last_time = current_time

                # THREE FINGERS -> PREVIOUS SLIDE
                elif total == 3:
                    send_keystroke("press", "left")
                ...
                # TWO FINGERS -> START PRESENTATION
                    ...
                    if platform.system() == "Darwin":
                        send_keystroke("hotkey", "command", "enter")
                    elif platform.system() == "Windows":
                        send_keystroke("hotkey", "ctrl", "f5")
                    else:
                        send_keystroke("hotkey", "command", "enter")
                ...
                # FIST -> EXIT PRESENTATION (0 fingers up)
                    ...
                    send_keystroke("press", "esc")
                    gesture = "Exit Slideshow"
                    last_time = current_time
                else:
                    gesture = gesture_details
            else:
                gesture = gesture_details
